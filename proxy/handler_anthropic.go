...
func (h *Handler) messagesOnce(c *gin.Context) {
	...
	for attempt := 0; ; attempt++ {
		...
		if isRelayAccount {
			...
			upstreamBody, _ := applyAccountFastTierPolicy(codexBody, account)
			if account.FastTierPolicy() != "preserve" {
		...
		if reqErr != nil {
			...
			if sameAccountRetry {
				...
				h.logSameAccountRetryRequestError(c, &database.UsageLogInput{
					...
					DurationMs: durationMs, ReasoningEffort: reasoningEffort, InboundEndpoint: "/v1/messages",
					UpstreamEndpoint: upstreamEndpoint, Stream: isStream, ViaWebsocket: useWebsocket,
					ServiceTier: usageTiers.ServiceTier, RequestedServiceTier: usageTiers.RequestedServiceTier,
					ActualServiceTier: usageTiers.ActualServiceTier, BillingServiceTier: usageTiers.BillingServiceTier,
				}, attempt, kind, reqErr)
			}
			if shouldPenalizeTransportKind(kind) && !(timedOut && shouldRetry) && !sameAccountRetry {
			...
			if shouldRetry {
				if sameAccountRetry {
					...
					logTransportSameAccountRetry(account.ID(), attempt+1, sameAccountFailures, sameAccountLimit, "/v1/messages")
				}
				continue
			}
			sendAnthropicError(c, http.StatusBadGateway, "api_error", "Upstream request failed")
		...
		if resp.StatusCode != http.StatusOK {
			...
			if cyberPolicy {
				sendAnthropicCyberPolicyError(c, http.StatusForbidden, upstreamCyberPolicyResponseMessage(c))
				return
			}
		...
		if isStream {
			...
			readErr = ReadSSEStream(resp.Body, func(data []byte) bool {
				...
				if eventType == "response.failed" && wroteAnyBody && writeErr == nil {
					failedOutcome := classifyResponseFailedOutcomeForAccount(account, terminalFailurePayload)
					if isExplicitUpstreamCyberPolicy(terminalFailurePayload) {
						...
						if err := writeAnthropicCyberPolicyStreamError(c, failedOutcome.failureMessage); err != nil {
							writeErr = err
						}
						return false
					}
		...
		if len(terminalFailurePayload) > 0 {
			...
			cyberPolicy := isExplicitUpstreamCyberPolicy(terminalFailurePayload)
			if cyberPolicy {
				outcome.failureKind = upstreamErrorKindCyberPolicy
				outcome.logStatusCode = http.StatusForbidden
		...
		if isStream && !wroteAnyBody && writeErr == nil && c.Request.Context().Err() == nil && outcome.logStatusCode != http.StatusOK {
			if outcome.failureKind == upstreamErrorKindCyberPolicy {
				...
				// 首包前上游失败、未向下游写过任何字节，按真实错误码返回 Anthropic JSON。
		...
		if !isStream {
			if outcome.failureKind == upstreamErrorKindCyberPolicy {
				sendAnthropicCyberPolicyError(c, http.StatusForbidden, outcome.failureMessage)
