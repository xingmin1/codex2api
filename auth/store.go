...
func normalizeFailureToleranceThreshold(value, fallback int) int {
	if value <= 0 {
		value = fallback
	}
	if value < 1 {
	...
	if value > maxFailureToleranceThreshold {
		return maxFailureToleranceThreshold
	}
	return value
}

func normalizeFailureToleranceOverride(value int) int {
	if value <= 0 {
		return 0
	}
...
// GetPriceMultiplier 返回账号级价格倍率；第二个返回值表示是否显式配置。
...
func NewStore(db *database.DB, tc cache.TokenCache, settings *database.SystemSettings) *Store {
	...
	if strings.TrimSpace(settings.TransportRetryPolicy) == "" {
		settings.TransportRetryPolicy = "hybrid"
		if settings.TransportSameAccountRetries == 0 {
			settings.TransportSameAccountRetries = defaultTransportSameAccountRetries
		}
	}
...
func (s *Store) wakeCheapProbe() {
	...
	if s.cheapProbeWakeCh == nil {
		s.cheapProbeWakeCh = make(chan struct{}, 1)
	}
	select {
...
func (s *Store) SetCheapProbeRankPolicy(base, step, minValue time.Duration) {
	...
	oldBase := time.Duration(atomic.LoadInt64(&s.cheapProbeRankBaseInterval))
	oldStep := time.Duration(atomic.LoadInt64(&s.cheapProbeRankStepInterval))
	oldMin := time.Duration(atomic.LoadInt64(&s.cheapProbeRankMinInterval))
	...
	if oldBase != 0 && (oldBase != base || oldStep != step || oldMin != minValue) {
...
func (s *Store) buildAccountFromRow(ctx context.Context, row *database.AccountRow, modelCooldowns map[int64][]*database.AccountModelCooldownRow) *Account {
	...
	if minutes, ok := row.GetCredentialInt64("cheap_probe_bonus_duration_minutes"); ok {
...
func (s *Store) NextForSessionWithFilter(key string, apiKeyID int64, exclude map[int64]bool, filter AccountFilter) (*Account, string) {
	...
	if !skipCachedAffinity {
		if binding, ok := s.getCachedSessionAffinity(key); ok {
			...
			// 跨进程缓存的 binding 也按 bounded 逻辑校验账号健康与有界生命周期；Grok 账号套用专属模式。
			cacheMode := mode
			if override := s.resolveGrokAffinityOverride(binding.accountID); override != "" {
				cacheMode = override
			}
...
func (s *Store) affinityProxyStillValid(accountID int64, proxyURL string) bool {
	...
	s.mu.RUnlock()
	if account == nil {
		return !poolEnabled && globalProxy == ""
	}
	...
	if poolEnabled {
		return poolHasEntries && poolContainsProxy
	}
	if globalProxy != "" {
		return proxyURL == globalProxy
	}
	// 旧缓存可能只有代理值而没有来源元数据；无可验证代理配置时兼容该值。
	return true
}
...
func (s *Store) SetFailureScoreThreshold(value int) {
	...
	s.failureScoreThreshold.Store(int64(normalizeFailureToleranceThreshold(value, defaultFailureScoreThreshold)))
...
func (s *Store) SetFailureCooldownThreshold(value int) {
	...
	s.failureCooldownThreshold.Store(int64(normalizeFailureToleranceThreshold(value, defaultFailureCooldownThreshold)))
	s.refreshFailureToleranceThresholds()
}
...
func (s *Store) ApplyAccountFailureToleranceConfig(dbID int64, scoreSet bool, scoreThreshold int, cooldownSet bool, cooldownThreshold int, windowSet bool, windowSeconds int) bool {
	...
	acc.mu.Lock()
	if scoreSet {
		acc.FailureScoreThresholdOverride = normalizeFailureToleranceOverride(scoreThreshold)
	}
	if cooldownSet {
...
// ApplyAccountUnauthorizedCooldownConfig 更新 401 冷却忽略开关。
...
// ApplyAccountPriceMultiplier 更新账号价格倍率并清空便宜探测运行态。
func (s *Store) ApplyAccountPriceMultiplier(dbID int64, priceMultiplier float64) bool {
	...
	acc.mu.Lock()
	acc.PriceMultiplier = normalized
...
// ApplyAccountCheapProbeConfig 更新账号级便宜探测恢复加分覆盖配置。
